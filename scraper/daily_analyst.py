"""
daily_analyst.py — Neural Scout Daily Performance Report Agent v2.0
====================================================================
Generiert täglich einen umfassenden 24h Performance-Report für alle Picks
der letzten 24 Stunden, inklusive:
  - Gesamtperformance (WR, ROI, P&L, Brier Score)
  - Market-Type Breakdown (Moneyline, Handicap, Totals)
  - Surface × Market-Type Analyse
  - Edge-Kalibrierung (korreliert Edge mit echten Ergebnissen?)
  - Stake-Effizienz (setzen wir mehr auf bessere Picks?)
...
def log(msg: str):
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [DailyAnalyst] {msg}")
...
def parse_value_from_text(text):
    """Extrahiert Pick-Info aus ai_analysis_text."""
    if not text:
...
def is_player1_target(pick_name: str, p1_name: str) -> bool:
    if not pick_name or not p1_name:
        return False
    ...
    if p1_last and len(p1_last) >= 2 and p1_last in pick_words:
        return True
    if p1_first and len(p1_first) >= 2 and p1_first in pick_words:
        return True
    return False


def check_play_result(pick_name: str, match: dict) -> bool:
    """Prüft ob ein Pick gewonnen oder verloren hat."""
    if not pick_name or not match:
        return False
    ...
    if "over" in lower_pick or "under" in lower_pick:
        ...
        if "over" in lower_pick:
            return total_games > boundary
        elif "under" in lower_pick:
            return total_games < boundary
        return False

    # Handicap
    elif re.search(r'[+-]\s*\d+(?:\.\d+)?', pick):
        if not score or not p1 or not p2:
            return False
        clean_score = re.sub(r'[^0-9\-\s]', '', score.replace(':', '-'))
        sets = clean_score.split()
        ...
        if not match_sign_num:
            return False
        handicap = float(match_sign_num.group(1).replace(' ', ''))
        if is_p1:
            return p1_games + handicap > p2_games
        elif is_p2:
            return p2_games + handicap > p1_games
        return False
    ...
    # Moneyline
        ...
        if w_last and len(w_last) > 2 and w_last in p:
            return True
        return False


def _classify_pick(pick_name):
    """Klassifiziert Markt-Typ und extrahiert Wert."""
    lo = (pick_name or "").lower()
    if "over" in lo:
        m = re.search(r"([\d.]+)", lo)
...
def compute_p_value(n_bets, avg_odds, roi):
...
async def call_openrouter(prompt, system_prompt, model=MODEL_NAME, temp=0.15):
    ...
    headers = {
        ...
        "X-Title": "NeuralScout Daily Analyst"
    }
        ...
        for current_model in models_to_try:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    log(f"⚠️ OpenRouter Error {current_model}: HTTP {response.status_code}")
...
async def run_daily_analysis():
    """Hauptfunktion: Generiert täglichen 24h Performance-Report."""
    log("📊 Starting Daily Performance Report Agent v2.0...")

    today_str = datetime.now(timezone.utc).strftime("%Y-%m-%d")

    # Prüfe ob Report heute schon existiert
    try:
        check_res = supabase.table("scout_reports").select("id").eq("report_date", today_str).execute()
        if check_res.data:
            log(f"ℹ️ Report für {today_str} existiert bereits. Überschreibe.")
            # Alten Report löschen
        ...
        if "relation" in err_msg or "does not exist" in err_msg or "PGRST204" in err_msg:
            log(f"⚠️ Table 'scout_reports' does not exist. Please run migration.")
            return

    # ═══ 1. ALLE SETTLED MATCHES DER LETZTEN 30 TAGE LADEN ═══
    cutoff_30d = (datetime.now(timezone.utc) - timedelta(days=30)).isoformat()
    try:
        res = supabase.table("market_odds")\
            .select("player1_name, player2_name, odds1, odds2, opening_odds1, opening_odds2, "
                    "ai_fair_odds1, ai_fair_odds2, actual_winner_name, score, created_at, "
                    "tournament, ai_analysis_text")\
    ...
    log(f"Loaded {len(matches)} settled matches from last 30 days.")

    # ═══ 2. RICH PICKS AUFBAUEN ═══
    rich_picks = []
    for m in matches:
        ...
        surface = "hard"
        if any(x in tournament for x in ["clay", "sand", "terre", "tierra", "erde"]):
            surface = "clay"
        elif any(x in tournament for x in ["grass", "rasen", "wimbledon", "eastbourne", "halle", "nottingham", "mallorca"]):
            surface = "grass"
    ...
    log(f"Built {total_rich} rich picks for analysis.")

    # ═══ 3. QUANTITATIVE BREAKDOWNS ═══
    ...
    for surf in ["hard", "grass", "clay"]:
        for ll in line_labels:
            ...
            if grp:
                ...
                hc_line_surf[f"{surf}|{ll}"] = {
                    "n": n, "wins": wins, "cov_pct": round(wins / n * 100, 1),
                    "pnl": round(pnl, 2), "roi": round(roi, 1), "avg_line": round(avg_line, 1)
                }

    # H. Totals Line × Surface
    ...
    for surf in ["hard", "grass"]:
        for tlb in ["<19.5", "19.5–20.5", "20.5–21.5", "21.5–22.5", "22.5–23.5", "23.5+"]:
            ...
            if len(grp) >= 3:
                ...
                pnl = sum(p["profit"] for p in grp)
                roi = pnl / staked * 100 if staked > 0 else 0
                total_line_surf[f"{surf}|{tlb}"] = {
                    "n": n, "hit_pct": round(wins / n * 100, 1),
                    "pnl": round(pnl, 2), "roi": round(roi, 1)
                }

    # I. Odds Bracket × Market Type
    odds_bkt_mkt = {}
    for mt in ["MONEYLINE", "HANDICAP_FAVE", "HANDICAP_DOG", "TOTAL_OVER"]:
...
Skill-Metrik: Skill = WR% minus Break-Even-WR bei dieser Quote.
...
"""
    for k, s in challenger_stats.items():
        if s and s["n"] >= 3:
            analytics_text += f"  {_fmt(k, s)}\n"

    # Beste/schlechteste Segmente
    critical_good = []
    critical_bad = []
    ...
    try:
        ...
        if autopilot_rule:
            ...
            log(f"🧠 Autopilot: Active={autopilot_enabled}, MaxVeto={max_veto_percentage}%, DrawdownLimit={drawdown_limit}u")
    except Exception as e:
        log(f"⚠️ Error loading scout rules: {e}")
        all_rules = []
    ...
    log(f"🧠 48h Portfolio Net Profit = {recent_profit_48h:+.2f}u (Limit: -{drawdown_limit}u)")
    if recent_profit_48h <= -drawdown_limit:
        drawdown_triggered = True
        log(f"🚨 EMERGENCY SHUTDOWN: 48h Drawdown Limit exceeded!")
        if autopilot_enabled:
            ...
            if autopilot_rule:
                try:
                    ...
                    log("🚨 Autopilot AUTOMATICALLY DISABLED.")
                except Exception as e:
                    log(f"⚠️ Error disabling autopilot: {e}")

    # Rule Evaluation
    def _matches_rule_conditions(pick, conditions):
        if "surface" in conditions:
            if conditions["surface"].lower() != pick["surface"].lower():
                return False
        if "is_favorite" in conditions:
            if conditions["is_favorite"] != pick["is_fav"]:
                return False
        if "is_challenger" in conditions:
            if conditions["is_challenger"] != pick["is_challenger"]:
                return False
        if "tour" in conditions:
            if conditions["tour"].upper() != pick["tour"].upper():
                return False
        ...
        if recommended_status and action_type:
            ...
            if autopilot_enabled:
                try:
                    ...
                    auto_executed = True
                    log(f"⚡ Autopilot: Changed '{desc}' to {recommended_status}.")
                except Exception as db_err:
                    log(f"⚠️ Autopilot error: {db_err}")
            rule_recommendations.append({
                "rule_id": rule_id, "description": desc,
                "current_status": rule_status, "recommended_status": recommended_status,
                "action_type": action_type, "bets": mc,
                "roi": round(roi_val, 1), "profit": round(profit_val, 2),
                "p_value": p_val, "auto_executed": auto_executed, "reason": reason
            })

    # Rules Summary für LLM
    drawdown_warning_md = ""
    ...
    - "line_outside_range": [low, high] (z.B. [2.5, 3.5] - vetoed falls Linie außerhalb des Bereichs)
    ...
    - "max_stake_cap": float (z.B. 2.0 - maximale Stake für multiplier-Regel)

    ANTWORTE NUR MIT EINEM VALIDEN JSON-ARRAY NACH DEM REPORTTEXT, FORMATIERT ALS 'PROPOSALS_JSON:'
    PROPOSALS_JSON:
    [
      {{
    ...
    net_profit = sum(p["profit"] for p in rich_picks)
    wins_count = sum(1 for p in rich_picks if p["is_win"])
    losses_count = len(rich_picks) - wins_count
    win_rate = (wins_count / len(rich_picks)) * 100 if rich_picks else 0.0
    ...
    for p in rich_picks:
        if p["fair"] > 1.0:
            prob_fair = 1.0 / p["fair"]
            outcome_val = 1.0 if p["is_win"] else 0.0
            brier_sum += (prob_fair - outcome_val) ** 2
            brier_count += 1
    avg_brier = brier_sum / brier_count if brier_count > 0 else 0.0

    # 24h Stats
    today_profit = sum(p["profit"] for p in picks_24h) if picks_24h else 0.0
    today_wins = sum(1 for p in picks_24h if p["is_win"]) if picks_24h else 0
    today_losses = len(picks_24h) - today_wins if picks_24h else 0
    today_win_rate = (today_wins / len(picks_24h)) * 100 if picks_24h else 0.0
    ...
    metrics_breakdown = {
        ...
        "handicap_lines": hc_line_surf,
        "totals_lines": total_line_surf,
        "odds_brackets": odds_bkt_mkt,
        "weekly_trend": weekly_trend,
        "challenger_vs_main": challenger_stats,
        "critical_good": critical_good[:6],
        "critical_bad": critical_bad[:6],
    }
    ...
    log(f"📊 SUMMARY: {len(rich_picks)} picks analyzed, WR={win_rate:.1f}%, ROI={roi:+.1f}%, P&L={net_profit:+.2f}u, Brier={avg_brier:.4f}")
    log(f"📊 24H: {len(picks_24h)} picks, WR={today_win_rate:.1f}%, P&L={today_profit:+.2f}u")
