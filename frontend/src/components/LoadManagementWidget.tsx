...
import { HeartPulse, Battery, AlertTriangle } from 'lucide-react';
...
export const LoadManagementWidget: FC<LoadManagementWidgetProps> = ({
  ...
  let statusText = t('loadManagement.statusText.fresh', 'FRESH');
  ...
  let description = t('loadManagement.descriptions.fresh', 'Optimal physical condition. No signs of fatigue expected.');

  const percentage = Math.min(100, Math.max(0, (fatigueMins / 1200) * 100));

  if (fatigueMins > 900) {
    statusText = t('loadManagement.statusText.critical', 'CRITICAL LOAD');
    statusColor = 'text-rose-500';
    bgColor = 'bg-rose-500';
    shadowColor = 'shadow-[0_0_15px_rgba(244,63,94,0.3)]';
    borderClass = 'border-rose-500/20';
    description = t('loadManagement.descriptions.critical', 'Extreme fatigue. Massive risk of performance drop in late sets.');
  } else if (fatigueMins > 600) {
    statusText = t('loadManagement.statusText.heavy', 'HEAVY LEGS');
    statusColor = 'text-amber-500';
    bgColor = 'bg-amber-500';
    shadowColor = 'shadow-[0_0_15px_rgba(245,158,11,0.2)]';
    borderClass = 'border-amber-500/20';
    description = t('loadManagement.descriptions.heavy', 'Increased load over the last 2 weeks. Recovery deficit likely.');
  } else if (fatigueMins > 300) {
    statusText = t('loadManagement.statusText.rhythm', 'MATCH RHYTHM');
    statusColor = 'text-sky-400';
    bgColor = 'bg-sky-400';
    shadowColor = 'shadow-[0_0_15px_rgba(56,189,248,0.2)]';
    borderClass = 'border-sky-400/20';
    description = t('loadManagement.descriptions.rhythm', 'Perfect match rhythm. Player is dialed in without being overworked.');
  }
      ...
      <div className={`bg-[#151821]/80 backdrop-blur-md rounded-3xl p-6 border ${borderClass} shadow-xl relative overflow-hidden group`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-white font-black text-sm uppercase tracking-wider flex items-center gap-2">
            ...
            {t('loadManagement.title', 'Physiological Load Status')}
          </h3>
          ...
          <div className="md:col-span-2 space-y-4">
            <div className="flex justify-between items-end">
              <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                {t('loadManagement.timeOnCourt', 'Time on Court (Last 14 Days)')}
              </span>
              <span className="text-2xl font-black text-white leading-none">
                {fatigueMins} <span className="text-sm text-gray-500 ml-0.5">{t('loadManagement.min', 'Min')}</span>
              </span>
            </div>
          ...
          <div className="bg-black/20 rounded-2xl p-4 border border-white/5 flex items-start space-x-2">
            ...
            <div>
              <span className="text-[9px] font-black uppercase tracking-widest text-gray-500 block mb-0.5">
                {t('loadManagement.assessment', 'Load Assessment')}
              </span>
          ...
          <div className="flex justify-between items-start">
            <div className="space-y-1">
              <span className="text-[9px] font-black uppercase tracking-wider text-gray-500 block">
                {t('loadManagement.comebackRating', 'Comeback Rating')}
              </span>
            ...
            <div className="text-right">
              <span className="text-[9px] font-black uppercase tracking-wider text-gray-500 block">
                {t('loadManagement.post1stSetLoss', 'Post 1st-Set Loss')}
              </span>
              <span className="text-sm font-black text-white block mt-1">
                {t('loadManagement.statsSummary', '{{wins}}W – {{losses}}L ({{rate}}%)', { 
                  wins: comebackStats.wins, 
                  losses: comebackStats.total - comebackStats.wins, 
                  rate: comebackStats.rate 
                })}
              </span>
            </div>
          </div>
          <p className="text-[11px] text-gray-400 mt-3 leading-normal border-t border-white/5 pt-3">
            {t('loadManagement.comebackDesc', 'Measures the player\'s capacity to salvage a victory after losing the opening set, calculated from their recent match history logs.')}
          </p>
        </div>
      )}
    </div>
