...
export function LegalModal({ type, onClose }: { type: string; onClose: () => void }) {
  const contentMap: Record<string, { title: string; icon: any; body: JSX.Element }> = {
    ...
    affiliate: {
      title: "Partner & Affiliate Agreement",
      icon: Briefcase,
      body: (
        <div className="space-y-6 text-gray-400 text-sm leading-relaxed">
          <section>
            <p className="text-white font-black uppercase text-xs mb-2 tracking-widest underline decoration-tennis-lime decoration-2">1. Nature of the Relationship (Independent Contractor)</p>
            <p>By activating a Partner Account, you ("Affiliate") enter into a commercial marketing agreement with BACKHAND.DTL. The Affiliate acts strictly as an independent contractor. This agreement does not create an employment relationship, partnership, or joint venture. The Affiliate is solely responsible for paying all applicable taxes (including income and business taxes) on commissions earned.</p>
          </section>
          <section>
            <p className="text-white font-black uppercase text-xs mb-2 tracking-widest underline decoration-tennis-lime decoration-2">2. Commission & Tracking Mechanics</p>
            <p>Commissions are exclusively tracked via the Affiliate's unique Promo Code during the checkout process managed by our Merchant of Record (Lemon Squeezy). BACKHAND.DTL pays a 30% recurring commission on net revenue (excluding VAT/Sales Tax) generated by the Affiliate's code. Commissions are paid on the 15th of the following month, provided the minimum threshold of €25.00 is met. Refunds or chargebacks will result in a deduction of the corresponding commission from the Affiliate's balance.</p>
          </section>
          <section>
            <p className="text-white font-black uppercase text-xs mb-2 tracking-widest underline decoration-tennis-lime decoration-2">3. Self-Billing / Gutschriftverfahren (§ 14 Abs. 2 UStG)</p>
            <p>To facilitate a frictionless payout process, both parties agree to the <strong>Self-Billing Procedure (Gutschriftverfahren)</strong> in accordance with § 14 Abs. 2 UStG (German VAT Act). <br/><br/>
            <strong>Agreement:</strong> BACKHAND.DTL will issue credit notes (Gutschriften) on behalf of the Affiliate for the commissions earned. The Affiliate is relieved of the obligation to issue invoices. The credit note is deemed accepted if not objected to in writing within 14 days of receipt.<br/>
            <strong>Taxation:</strong> For Affiliates residing outside of Germany but within the EU (B2B) or outside the EU, the Reverse-Charge mechanism applies, and the tax liability shifts to BACKHAND.DTL. Affiliates residing in Germany must inform BACKHAND.DTL if they are subject to standard VAT or the small business regulation (Kleinunternehmerregelung).</p>
          </section>
          <section>
            <p className="text-white font-black uppercase text-xs mb-2 tracking-widest underline decoration-tennis-lime decoration-2">4. Anti-Spam & Brand Safety (Zero Tolerance)</p>
            <p>Affiliates are strictly prohibited from engaging in "Brand Bidding" (bidding on branded keywords like "BACKHAND.DTL" in Google Ads/Search). Spamming in competitor forums or presenting the platform as a "get rich quick" betting scheme is forbidden. Violation of these rules will result in an immediate lifetime ban and forfeiture of all pending commissions.</p>
          </section>
        </div>
      )
    },
        ...
        <footer className="w-full border-t border-white/5 bg-[#0a0b0e] py-8 mt-auto relative z-20 pb-32 md:pb-8">
            <div className="max-w-7xl mx-auto px-4 md:px-8 flex flex-col md:flex-row items-center justify-between gap-4 text-center md:text-left">
                ...
                <div className="flex flex-wrap items-center justify-center gap-4 md:gap-6 text-[10px] font-black uppercase tracking-widest text-gray-500">
                    ...
                    <button onClick={() => onOpenLegal('affiliate')} className="hover:text-tennis-lime transition-colors">Affiliate Terms</button>
